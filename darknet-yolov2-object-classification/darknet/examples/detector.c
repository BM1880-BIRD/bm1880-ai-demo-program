...
void test_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, char *outfile, int fullscreen)
{
    ...
    if (!filename) {
        printf("Could not open current directory, please input image directory name.\n" ); 
        return;
    }
    ...
    while ((files_in_dir = readdir(dir)) != NULL) {
        if(strstr(files_in_dir->d_name, "jpg") == NULL) {
            continue;
        }
        ...
        strcat(input, "/");
        strcat(input, files_in_dir->d_name);
        ...
        if(outfile){
            ...
            cvResizeWindow("predictions", 640, 480);
            if(fullscreen){
                cvSetWindowProperty("predictions", CV_WND_PROP_FULLSCREEN, CV_WINDOW_FULLSCREEN);
            }
            // show_image(im, "predictions", 0);
            show_image(im, "predictions", 10);
#endif
        }
        free_image(im);
        free_image(sized);
