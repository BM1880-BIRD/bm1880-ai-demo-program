...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[bmnet]")==0) return BMNET;
    return BLANK;
}
...
layer parse_yolo(list *options, size_params params)
{
    ...
    int input_w = option_find_int_quiet(options, "input_w", -1);
    int input_h = option_find_int_quiet(options, "input_h", -1);
    int h = (input_h == -1 ) ? params.h : input_h;
    int w = (input_w == -1 ) ? params.w : input_w;
    layer l = make_yolo_layer(params.batch, w, h, num, total, mask, classes);
    l.input_offset = option_find_int_quiet(options, "input_offset", 0);
...
layer parse_bmnet(list *options, size_params params)
{
    int out_h = option_find_int(options, "out_h",1);
    int out_w = option_find_int(options, "out_w",1);
    int out_c = option_find_int(options, "out_c",1);
    int out_num = option_find_int(options, "out_num",1);
    int is_int8 = option_find_int(options, "is_int8", 0);
    unsigned long long output_offset = option_find_ull(options, "output_offset", 0);
    unsigned long long neuron_size = option_find_ull(options, "neuron_size", 0);
    char *weight_file = option_find_str(options, "weight_file", NULL);
    char *cmdbuf_file = option_find_str(options, "cmdbuf_file", NULL);
    float in_threshold = option_find_float(options, "in_threshold", 0);
    int batch,h,w,c;
    ...
    c = params.c;
    batch=params.batch;
    if(!(h && w && c)) error("Layer before bmnet layer must output image.");

    bmnet_layer layer = make_bmnet_layer(
        batch, h, w, c, out_h, out_w, out_c, output_offset,
        neuron_size, weight_file,
        cmdbuf_file, is_int8, in_threshold);

    layer.out_num = out_num;
    option_find_float_list(options, "sub_output_threshold", layer.sub_output_threshold, out_num);
    option_find_int_list(options, "sub_output_size", layer.sub_output_size, out_num);
