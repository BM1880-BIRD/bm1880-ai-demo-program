...
mp::optional<vector<Memory::SharedView<unsigned char>>> DemuxStream::get() {
...
bool DemuxStream::put(vector<Memory::SharedView<unsigned char>> &&data) {
...
void DemuxStream::push_read_data(vector<Memory::SharedView<unsigned char>> &&data) {
    unique_lock<mutex> locker(read_mutex_);
...
bool DemuxStream::send(const DemuxHeader &header, std::vector<Memory::SharedView<unsigned char>> &&data) {
    data.insert(data.begin(), name_data_);
    data.insert(data.begin() + 1, Memory::SharedView<DemuxHeader>(header));
...
void PacketDemux::demux() {
    ...
    if (!data->empty()) {
        string key;
        data->erase(data->begin(), decode(data->begin(), data->end(), key));
        ...
        if (!data->empty()) {
            auto header = data.value()[0].slice_front<DemuxHeader>(1);
            ...
            if (header.size() == 1) {
                ...
                case DemuxHeader::Control:
                    stream->set_peer_capacity(header.data()->buffer_capacity);
                    break;
