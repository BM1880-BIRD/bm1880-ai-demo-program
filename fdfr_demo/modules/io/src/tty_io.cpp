...
constexpr size_t MaxDataLength = MaxPacketLength - PacketPadidngLength - sizeof(PacketHeader);
...
TtyIO::TtyIO(const TtyConfig &config) : closing_(false), my_process_id(time(NULL)) {
    open(config);
...
mp::optional<std::vector<Memory::SharedView<unsigned char>>> TtyIO::get() {
...
bool TtyIO::put(vector<Memory::SharedView<unsigned char>> &&obj) {
    if (!is_opened()) {
        return false;
    }

    Memory::View<uint32_t> sizes;
    sizes.push_back(obj.size());
    for (auto &segment : obj) {
        sizes.push_back(segment.size());
    }
    obj.insert(obj.begin(), sizes.to_shared());
    ...
    for (size_t offset = 0; offset < total_size; offset += MaxDataLength) {
        queue_output(out_locker, session_id, PacketHeader::DATA, total_size, offset, min(total_size - offset, MaxDataLength));
    }

    write_sessions_[session_id].total_size = total_size;
    write_sessions_[session_id].pending.ranges = {Memory::Range{0, total_size}};
...
void TtyIO::handle_read() {
...

    {
        ...
        if (pending_sessions.empty() && header.min_write_session_id != UINT32_MAX) {
            pending_sessions.ranges = {Memory::Range{header.min_write_session_id, UINT32_MAX}};
        }
    }

    switch (header.type) {
    case PacketHeader::DATA:
        handle_data(header);
        break;
...
void TtyIO::generate_output() {
    ...
    if (output_queue_.empty()) {
        ...
        for (auto &session : write_sessions_) {
            for (auto &range : session.second.pending.ranges) {
                for (size_t offset = range.begin; offset < range.end; offset += MaxDataLength) {
                    queue_output(out_locker, session.first, PacketHeader::DATA, session.second.total_size, offset, min(range.end - offset, MaxDataLength));
                    return;
                }
            }
        }
        for (auto &session : read_sessions_) {
            for (auto &range : session.second.pending.ranges) {
                for (size_t offset = range.begin; offset < range.end; offset += MaxDataLength) {
                    queue_output(out_locker, session.first, PacketHeader::REQUEST, session.second.data.size(), offset, min(range.end - offset, MaxDataLength));
                    return;
                }
            }
        }
...
void TtyIO::handle_write() {
    ...
    if (!output_queue_.empty()) {
        PacketHeader header = output_queue_.front();
        output_queue_.pop();
        out_locker.unlock();
        ...
        if (header.type == PacketHeader::DATA) {
            out_locker.lock();
            auto iter = write_sessions_.find(header.session_id);
            if (iter != write_sessions_.end()) {
                file_.write((unsigned char*)&header, sizeof(PacketHeader));
                if (header.type == PacketHeader::DATA) {
                    ...
                    for (auto &segment : iter->second.data) {
                        if (length <= 0) {
                            ...
                            file_.write(segment.data() + offset, write_length);
                            length -= write_length;
                            offset = 0;
                        }
                    }
        ...
        } else if (header.type == PacketHeader::REQUEST) {
            lock_guard<mutex> in_locker(in_lock_);
...
bool TtyIO::read_header(PacketHeader &header) {
    size_t offset = read_buffer_.size();
    read_buffer_.resize(offset + sizeof(PacketHeader));
    ...
    auto search_end = read_buffer_.end() - sizeof(PacketHeader) + PacketHeader::MagicLength::value;
    auto ret = search(read_buffer_.begin() + offset - sizeof(PacketHeader), search_end,
                      PacketHeader::header_magic, PacketHeader::header_magic + PacketHeader::MagicLength::value);
    if (ret == search_end) {
        return false;
    } else {
        offset = distance(read_buffer_.begin(), ret);
        assert(read_buffer_.size() >= offset + sizeof(PacketHeader));
        memcpy(&header, read_buffer_.data() + offset, sizeof(PacketHeader));
        file_.unread(read_buffer_.data() + offset + sizeof(PacketHeader),  read_buffer_.size() - offset - sizeof(PacketHeader));
        read_buffer_.clear();
        return true;
    }
}

map<uint32_t, ReadSession>::iterator TtyIO::create_read_session(const PacketHeader &header) {
    auto iter = read_sessions_.find(header.session_id);
    if (iter == read_sessions_.end()) {
        iter = read_sessions_.emplace(header.session_id, ReadSession{}).first;
        iter->second.data.resize(header.total_length);
        iter->second.pending.ranges = {Memory::Range{0, header.total_length}};
    }
    return iter;
}

void TtyIO::handle_data(const PacketHeader &header) {
    lock_guard<mutex> in_locker(in_lock_);
    ...
    if (!pending_sessions.contains(header.session_id) || source_data_.count(header.session_id) > 0) {
        ...
        queue_output(header.session_id, PacketHeader::ACK, header.total_length, 0, header.total_length);
        return;
    }
    ...
    iter->second.pending -= Memory::Range{header.data_offset, header.data_offset + bytes_read};

    queue_output(header.session_id, PacketHeader::ACK, header.total_length, header.data_offset, bytes_read);
...
void TtyIO::handle_ack(const PacketHeader &header) {
    lock_guard<mutex> out_locker(out_lock_);

    auto iter = write_sessions_.find(header.session_id);
    if (iter != write_sessions_.end()) {
        iter->second.pending -= Memory::Range{header.data_offset, header.data_offset + header.data_length};
        if (iter->second.pending.ranges.empty()) {
...
void TtyIO::handle_request(const PacketHeader &header) {
    lock_guard<mutex> out_locker(out_lock_);

    auto iter = write_sessions_.find(header.session_id);
    if (iter != write_sessions_.end()) {
        queue_output(out_locker, header.session_id, PacketHeader::DATA, header.total_length, header.data_offset, header.data_length);
    }
}

void TtyIO::queue_output(lock_guard<mutex> &, uint32_t session_id, PacketHeader::PacketType type, uint32_t total_length, uint32_t data_offset, uint32_t data_length) {
...
void TtyIO::queue_output(uint32_t session_id, PacketHeader::PacketType type, uint32_t total_length, uint32_t data_offset, uint32_t data_length) {
    lock_guard<mutex> out_locker(out_lock_);
    queue_output(out_locker, session_id, type, total_length, data_offset, data_length);
}

vector<Memory::SharedView<unsigned char>> TtyIO::slice_segments(vector<unsigned char> &&input) {
    Memory::SharedView<unsigned char> data(std::move(input));
    auto count = data.slice_front<uint32_t>(1);
    auto sizes = data.slice_front<uint32_t>(*count.begin());
    vector<Memory::SharedView<unsigned char>> segments;
    for (auto &size : sizes) {
        segments.emplace_back(data.slice_front<unsigned char>(size));
    }
    return segments;
}
