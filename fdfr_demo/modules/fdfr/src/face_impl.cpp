...
Memory::SharedView<fr_result_t> FaceImpl::faceid(const image_t &image) {
...
Memory::SharedView<fr_result_t> FaceImpl::faceinfo(const image_t &image ,double* score){
    vector<face_info_t> face_infos;
    Memory::View<fr_result_t> face_result;
    vector<face_features_t> features_list;
    fdfr_.detect_extract(image, face_infos, features_list);
...
Memory::SharedView<fd_result_t> FaceImpl::detect_face(const image_t &image) {
    vector<fd_result_t> face_infos;
    fdfr_.detect(image, face_infos);
    return Memory::SharedView<fd_result_t>(std::move(face_infos));
}
