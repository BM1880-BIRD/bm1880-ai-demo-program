...
public:
    ...
    memory::Iterable<fr_result_t> faceid(const image_t &image) override;
    bool register_face(const std::string &name, image_t &&image) override;
    memory::Iterable<fd_result_t> detect_face(const image_t &image) override;
    memory::Iterable<fr_result_t> faceinfo(const image_t &image ,double* score);
