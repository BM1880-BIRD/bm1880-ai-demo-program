...
AFSClassifyHSVYCbCr::AFSClassifyHSVYCbCr(const json &config) : AFSBase(config) {
    ...
    core.reset(new qnn::vision::AntiFaceSpoofingClassifyHSVYCbCr(config.at("model").get<string>()));
    core->SetThreshold(threshold);
}

result_item_t AFSClassifyHSVYCbCr::getItem(std::tuple<image_t &, VectorPack &> tuple) {
    result_item_t item;
    auto &vp = get<1>(tuple);
