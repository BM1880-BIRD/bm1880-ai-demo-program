...
public:
    ...
    bool put(std::list<memory::Bytes> &&data) override;
    void close() override;

protected:
    void push_read_data(std::list<memory::Bytes> &&data);
    void set_peer_capacity(uint32_t peer_capacity);

private:
    bool send(const DemuxHeader &header, std::list<memory::Bytes> &&data);

    std::string name_;
    memory::Bytes name_data_;
    ...
    std::queue<std::list<memory::Bytes>> read_data_;
