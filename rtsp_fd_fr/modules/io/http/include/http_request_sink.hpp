...
bool http_post_content(const std::string &addr, int port, const std::string &url, const std::string &mimetype, const memory::Bytes &content);
...
public:
    ...
    bool put(Ts &&...args) override {
        std::vector<std::tuple<std::string, std::string, memory::Bytes>> data;
...
private:
    ...
    void encode(std::vector<std::tuple<std::string, std::string, memory::Bytes>> &out, U &&u, Us &&...us) {
        out.emplace_back(names_[out.size()], HttpEncode<U>::mimetype(), HttpEncode<U>::encode(std::move(u)));
        encode(out, std::move(us)...);
    }
    void encode(std::vector<std::tuple<std::string, std::string, memory::Bytes>> &out) {}
